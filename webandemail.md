...
### Introduction
All organisations use email in some form and most have an online presence. This moudule introduces some of the management features for popular email tools and also recommends a number of methods for mitigating website hosting risks.
...
Where possible, it would be best for the trainer to have acces to trial or demo versions of the tools that are likely to be used in the module. Ideally participants will be able to then login and test them for themselves - without causing disruption to live systems. For example, G-Apps Suite, Protonmail, Wordpress etc.
...
For those who have implementated PGP within their own organisations. 

* How have did they do it? 
* Is it an individual responsibility? (E.g each user creates and manages their own PGP mail, shares keys)
...
* For some attacks, it may notify individuals if they have possibly been the vitim of "government-backed attackers"
...
*Office 365*
Office 365 administration offers a number of security features in it's administrator settings, such as:

* Easily add and remove users
* Reset passwords (both individual and in bulk)
* Set password expiration policy
* Create with different permissions (staff, administators etc)
...
* Good mobile apps
* Can be whitelabelled / used with organisation's domain
* Less associated with activism then well known services such as Hushmail or Riseup
* Ability to send an encrypted mail to an individual not on the service (Tutanota allows a plaintext email to be sent with a link to a password protected email - the password must be sent through another channel)

Disadvantages

* They are closed, proprietry systems. Only mail between account holders is encrypted. Users must trust the company.
...
* No verification of email addresses (easy to send a unencrypted email to a non service account

Virtu is another secure mail service that provides integrates into existing Google or Microsoft email products. It's encryption is also proprietry.
...
* It's mobile apps for viewing encrypted mails are of poor quality
* It only integates with Microsoft Exchange Server or IBM Domino
...
Civil society groups are often targets for email abuse. Often this can lead to the spread of spam from an organisations domain which can lead to emails being blocked. To prevent this, an organisation should setup a Sender Policy Framework (SPF) and DomainKeys Identified Mail (DKIM). 

* A SPF record allows the recieving server of a mail to check that it orginated from the an authorised host.
...
Most email providers allow you to setup both of these services. For G-Suite you can find out how to setup [SPF](https://support.google.com/a/answer/178723?hl=en) and [DKIM](https://support.google.com/a/answer/174124?hl=en).

If sending mail to a large amount of people (for example a newsletter), it is better to either send from a different domain/subdomain (e.g securityletter.com instead of security.com). Alternatively, using a services such as [Mailchimp](https://mailchimp.com/) or [GetResponse](https://www.getresponse.com/). 
...
*Wordpress*     
Wordpress is a very common platform for organisations to host their websites. In 2016, 60% of websites using a Content Management System used it (Joomla was 7% and Drupal 5%). Because of it's popularity, it is regularly targetted for attack. 
...
* If using a security plugin such as Wordfence Security, it is best not to run another similar plugin, as they can often cause problems when run together
...
*Website Security*  
All websites and web applications can potentially be at risk from a variety of possible attack methods. While there are many ways to setup and run a web site or application securely, there are ten top mitigations that are recommended for best practice.
